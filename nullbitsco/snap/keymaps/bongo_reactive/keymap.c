...
uint8_t persist_nkro_oled_state     = 0;
uint8_t persist_gui_swap_oled_state = 0;

static void render_status(void) {
    oled_set_cursor(0, 0);
    oled_write_P(PSTR("max&cheese "), false);
    ...
    if (persistent_led_state != led_usb_state || keymap_config.nkro != persist_nkro_oled_state) {
        persistent_led_state    = led_usb_state;
        persist_nkro_oled_state = keymap_config.nkro;
